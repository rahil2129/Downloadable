 ...
 * ID:            %!uniqueid!%
 * Last Modified: 2022-04-24T18:22:00+00:00
 * File:          Helper/Server.php
...
class Server extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        $url = str_replace(['http://', 'https://', 'www.'], '', (string)$this->scopeConfig->getValue('web/unsecure/base_url'));
        $url = explode('/', $url);
        $url = array_shift($url);
    ...
    {
        $url = str_replace(['http://', 'https://', 'www.'], '', (string)$this->_request->getServer('SERVER_NAME'));
        $url = explode('/', $url);
        $url = array_shift($url);
        $parsedUrl = parse_url((string)$url, PHP_URL_HOST);
        if ($parsedUrl !== null) {
            return $parsedUrl;
        }
        return $url;
    ...
    public function confirm(array $configuration, $updateConfiguration = false)
    {
        ...
        $s = trim((string)$this->scopeConfig->getValue($configuration['config_path'] . 'serial'));
